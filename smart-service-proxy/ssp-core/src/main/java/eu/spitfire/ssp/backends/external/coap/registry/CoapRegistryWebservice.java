...
/**
 * The {@link eu.spitfire.ssp.backends.external.coap.registry.CoapRegistryWebservice} processes incoming
...
public class CoapRegistryWebservice extends NotObservableWebservice<Void> {
     ...
     *                         {@link eu.spitfire.ssp.backends.external.coap.registry.CoapRegistry} from.
     */
    ...
    public void processCoapRequest(final SettableFuture<CoapResponse> registrationResponseFuture,
        ...
        try{
            ...
            if (coapRequest.getUriQuery().isEmpty()) {
            	...
            	if (query.startsWith(QUERY_ADD)) {
            		String webservicePath = query.replaceFirst(QUERY_ADD, "");
            		
            		URI serviceUri = new URI("coap", null, remoteAddress.getHostName(), 5683, webservicePath, null, null);
            		
            		log.info("Received registry add command for " + serviceUri);
            		
                    CoapWebservice coapWebservice = new CoapWebservice(serviceUri);
                    
                    registry.registerDataOrigin(coapWebservice);
            	}
            	else if (query.startsWith(QUERY_REMOVE)) {
            		String webservicePath = query.replaceFirst(QUERY_REMOVE, "");
            		
            		URI serviceUri = new URI("coap", null, remoteAddress.getHostName(), 5683, webservicePath, null, null);
            		
            		log.info("Received registry remove command for " + serviceUri);
            		
            		registry.unregisterDataOrigin(serviceUri);
            	}
            }
        }
        catch(Exception ex){
    ...
    private ListenableFuture<Set<URI>> getAvailableWebservices(final InetAddress remoteAddress) throws Exception {
        ...
        log.debug("Sent request to /.well-known/core");
                    ...
                    public void onSuccess(Multimap<String, LinkAttribute> result) {
                        try{
                            ...
                            for(String servicePath : result.keySet()){
                            	log.debug("found service: " + servicePath);
